...
# ---------------------------
# Parameter bounds & scaling (D,w,t,mat_lug,mat_pin)
# ---------------------------
...
def objective(x_scaled, penalize_factor=2e4, debug=False):
    ...
    # backplate
    marginbackplate, m_backplate = backplate(D2,x,h,t,t2, rho, Fty, n_f,w) # n_f from 2 to eight 
    ...
    print(margin_shear)
    m_fastener, L_sha2 = mass_fastner(t2, t3, D2, n_f, phi, Fx)
...
def optimize_cma(popsize=12, seed=0, maxiter=1000):
    ...
    marginbackplate, m_backplate = backplate(D2,x,h,t,t2,rho,Fty,n_f,w)
    m_fastener, L_sha2 = mass_fastner(t2, t3, D2, n_f, phi, Fx)
    ...
    fasteners = ForceAtFastener(fasteners,part_cg_y,n_f) # Calculate force at each fastener ------ took part_cg_y here do not know wether that is correct
    fasteners = ShearInPlates(fasteners, t2, t3, D2) # Calculate shear stresss at s/c wall and lug backplate
    backplate_pull_margin, sc_pull_margin = ShearMargins(fasteners, Fty)

    margin_bending = WorstBackplateMargin(D,t2,D2,Fty,w)
    margin_fastener_shear = fastener_shear(D2,n_f,x,t,w,h)
    margin_shear = WorstShearMargin(fasteners, Fty)
    

    return {
    'scaled': best_scaled,
    'w': w, 't': t, 'D': D, 'd_req': d_req,
    't2': t2, 'D2': D2, 'n_f': n_f,
    'x': x, 'h': h, 'phi': phi,  
    'mat_lug': mat_lug, 'mat_pin': mat_pin,
    'mass_lug': m_lug, 'mass_pin': m_pin,
    'mass_backplate': m_backplate,
    'margin_backplate': marginbackplate,
    'mass_fastener': m_fastener,
    'mass_total': total_mass,
    'MS_out': MS, 'f': f_val, 'obj': best['obj'],
    'fasteners': fasteners,
    'margin_bending': margin_bending,
    'margin_fastener_shear': margin_fastener_shear,
    'L_sha2': L_sha2
}
...
if __name__ == '__main__':
    ...
    print(f" margin_bending = {res['margin_bending']:.6f}")
    print(f" margin_fastener_shear = {res['margin_fastener_shear']:.6f}")
    print(f" objective = {res['obj']:.6f}")
    print(f" fasteners = {res['fasteners']}")
    print(f" phi = {res['phi']:.6f}")  
    print(f" L_sha2 = {res['L_sha2']}")