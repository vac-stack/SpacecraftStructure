...
# --- 2. Determining Loads (User Modified) ---
F_X = 100.0  
F_Y = 100.0  
F_Z = 300.0 
T_X = 0.005   # Thickness of the wall the lugs attach to 
...
def objective(x_scaled, penalize_factor=1e4):
    ...
    try:
        ...
        # 2. Bending Margin 
        I_xx, _ = lg.Ixx_zz(w1, t1)
        _, _, sm_bending = lg.bending(I_xx, w2, F_Z, t2, n_l)
    ...
    # Mass Calculation (True Mass - User Modified)
...
def run_optimization(popsize=20, maxiter=300):
    ...
    return dict(zip(keys, unscale(es.result.xbest)))
...
if __name__ == '__main__':
    ...
    # Get final results for output 
    sm_f, d_out, m_size = lg.fastener_diameter(F_Z, n_l, n_f, TAU_MAX_F)
    I_xx_opt, _ = lg.Ixx_zz(p['w_1'], p['t_1'])
    _, _, sm_b = lg.bending(I_xx_opt, p['w_2'], F_Z, p['t_2'], n_l)
