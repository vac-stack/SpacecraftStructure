...
def objective(x, penalize_factor=1e10):
    ...
    try:
        # 1. Fastener Diameter - Corrected Input: requires F_X 
        sm_fastener, d_out, m_size = lg.fastener_diameter(F_X, F_Z, n_l, n_f, TAU_MAX_F)

        # 2. Bending Margin - Corrected Unpacking: returns 4 values 
        I_xxh = lg.Ixx(w1, t1)
        _, _, sm_bending, M = lg.bending(I_xxh, w2, F_Z, t2, n_l)

        # 3. Pull-through and Plate Shear (Vertical)
...
if __name__ == '__main__':
    ...
    I_xxh = lg.Ixx(w1, t1)
    _, _, sm_b, M_final = lg.bending(I_xxh, w2, F_Z, t2, n_l)
    f_list = lg.CreateFastenerList_v(n_f, l_dim, w1, d_out)
    f_list = lg.ForceAtFastener_v(f_list, w2, n_f, n_l, F_Z, F_Y)
    f_list = lg.bearing_stress_v(f_list, t1, T_X, d_out)
    ...
    for k, v in best_params.items():
        if k == 'n_f': print(f"{k:4}: {n_f} (Even enforced)")
        elif k == 'n_l': print(f"{k:4}: {n_l}")
        else: print(f"{k:4}: {v:.6f}")
    ...
    print(f"Combined Stress : {sm_combined_val:10.4f}")
    print("="*30)